...
5. 有一定paas与微服务开发和架构经验，对服务和架构有一定调研、设计与实施的能力。
6. 一直用linux、vim工作，有运维和实施的能力。
7. 有带小团队的经验。
...
## 了解
* 工具: eclipse、vscode、maven、npm、pm2、openvpn、<br/>
&emsp;&emsp;&nbsp; jenkins、kibana、jira、confluence、prometheus、grafana等
* 技能: python、canvas、webgl、css等
* 框架: java ssh、springmvc、springboot、lucene、shiro、mybatis、birt、<br/>
&emsp;&emsp;&nbsp; koa、socket.io、swig、tornado、django、open tracing、<br/>
&emsp;&emsp;&nbsp; jquery、fis、bootstrap、webpack、gulp、vue、angluar1、three.js、sim.js等
* 服务: redis、tidb、pgsql、oracle、neo4j、<br/>
&emsp;&emsp;&nbsp; docker、nginx、rabbitmq、nsq、kafka、tomcat等
...
3. 支付模块重构
4. 数据平台、etl工具
5. 部分架构升级(express向koa、es5向es7，拆分服务)
...
### 职务介绍
    ...
    数据平台
        vue, birt
        etl增量同步mongodb数据到pgsql, 数据仓库, 数据可视化
    架构升级
        调研es6, es7所有特性, 调研koa, 原型实现, 拆分出三方服务
...
### 工作介绍
    ...
    我负责大部分node.js代码和小部分前端页面重构任务。日均pv 80万左右，重构前压力大，重构后稳定，响应时间降到20-80ms。
    期间接触到很多前辈，认识和实践了互联网公司的工作方式。学习到一些百度对node.js的使用方案。
### 职务介绍
    node代理层
        request组件(认证、加密等), 路经acl, bigpipe异步, swig模板渲染, 埋点统计等
    前端
        三级菜单, 个人中心等
...
### 职务介绍
    权限框架
        封装shiro, 实现数据权限, 树形级联配置, 分布式缓存
    数据库框架
        适配数据库, 反射bean类预拼装sql子句, 反射bean对象拼装单表sql, 提供常用方法
    搜索
